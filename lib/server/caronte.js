...
(function () {
	...
	Caronte.prototype = {
		 ...
		 * A url to the wsdl file to register.
		 * @param {Array} soapMethods
		 * An array of soap methods to create sources for.
		 * @param {String} [url]
		 * An endpoint to connect to.
		 */
		registerWsdl: function(wsdlPath, soapMethods, url) {
			var SoapSource = this.sourceTypes.soap;
			soapMethods.forEach(function(soapMethod) {
				var sourceKey = soapMethod;
				var sourceParams = {
					wsdlPath: wsdlPath,
					soapMethod: soapMethod,
					url: url
				};
				this._sources[sourceKey] = new SoapSource(sourceParams);
			}, this);
		},
